...
void play()
{
	...
	gotoxy(x, y); //³]©wÁä½Lªº¦ì¸m

	// §Q¥ÎwhileÅýµ{¦¡¤£·|°h¥X¡A¨Ã¥B¥i¥H¤@ª½¤U«ü¥O
	while (1)
	{
		// ÀË¬d¬O§_Å¥µP,
		if (cb.isChampion() > 0)
		{
			check = 1;
			PrintCheck(cb.isChampion());  // ¦L¥XÅ¥µP°T¸¹
			gotoxy(x, y);
		}
		else
		{
		...
		ch = _getch(); // Àò¨úÁä½L¿é¤Jªº­^¤å¦r¡AÂà´«¦¨·L³n©w¸qªº¼Æ¦r
		if (ch == -32) // §PÂ_¬O§_¬°¤W¤U¥ª¥k 
		{
			ch = _getch();
			switch (ch) // ¤W¤U¥ª¥k
			{
